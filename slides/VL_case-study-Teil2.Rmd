...
- Tutorium mit Lara morgen um 14:15 Uhr in BBB
- Hilfe im Forum
- Video Tutorials 
  - klonen von Github (mit Github-Desktop)
  - RTutor Problem Set lokal ausführen (gleich wie auf der RStudio Cloud)
...
--
- Länder im Süden Deutschlands haben eine sehr niedrige Arbeitslosenquote (<4%)
--

- Ostdeutsche Bundesländer leider unter hohen Arbeitslosenquoten (>8%)
--

- Standardabweichung bei allen Bundesländern vergleichbar   
  - Es gibt hier vermutlich keine großen Ausreißer bei den Landkreisen

--
- Median liegt recht nahe am Mittelwert für die Bundesländern

.alert[Allgemein sehen wir hier sehr große Unterschiede zwischen Ost- und Westdeutschland!]
...
- Fast 3 Prozentpunkte niedriger in den westdeutschen Bundesländern
- Die Standardabweichung ist vergleichbar
- Der Median liegt in Westdeutschland sogar noch deutlicher unter dem Mittelwert als in Ostdeutschland
- Im **25% Quantil** in **Ostdeutschland** ist die Arbeitslosenquote bei **6,00%**
- In **Westdeutschland** ist das **75% Quantil** bei einer Arbeitslosenquote von **6,17%**!
...
- Bundesländer mit niedrigen Arbeitslosenquoten haben ein hohes BIP pro Kopf
- Ostdeutsche Bundesländer haben alle ein BIP pro Kopf < 30000€
- Westdeutsche Bundesländer haben alle ein BIP pro Kopf > 30000€
- Einige Bundesländer mit wenig, einige mit hohen Schulden pro Kopf
...
- Rote Datenpunkte Bade-Württemberg, fast alle unter dem Median in Westdeutschland
...
- Das Niveau des BIP pro Kopf ist in den ostdeutschen Bundesländern deutlich niedriger als in den westdeutschen. 
- Stadtkreis Ulm hat ein sehr hohes BIP pro Kopf, auch im Zeitablauf
- Das BIP Pro Kopf nimmt im Zeitablauf in Ostdeutschland zu, doch erreicht es mit durchschnittlich 28338€ den Wert, welchen die westdeutschen Landkreise durchschnittlich in 2006 hatten!
...
Paneldaten beim BIP, d.h. wir können:
...
.question[Gab es einen Anpassungsprozess des BIP pro Kopf?]
...
bip_zeitreihe_namen %>%
  ...
  ungroup() %>%
  group_by(ost_name, Jahr) %>%
  mutate( durchschnitt = mean(bip_wachstum, na.rm=TRUE)) -> bip_wachstum
...
- Kerndichteschätzer (übernächste Folie)
...
`r chunk_reveal(chunk_name = "bip4")`

```{r bip4, include=FALSE}
gesamtdaten %>%
  filter(bip_pro_kopf<100000) %>%
  group_by(ost_name) %>%
...
gesamtdaten %>%
  ...
  facet_grid(ost_name~.) +
  geom_vline(data = mittel2, aes(xintercept = durchschnitt), color = "blue") +
  geom_vline(data = mittel2, aes(xintercept = median), color = "darkgreen") +
  theme_bw() +
  labs(title = "Verteilung des BIP pro Kopf für Ost- und Westdeutschland",
       subtitle = "Beobachtungen auf Landkreisebene in 2017, Durchschnitt = blau,\nMedian = grün",
       x = "BIP pro Kopf",
       y = "Dichte")
...
Kendichteschätzer und Histogramm bestätigen das Bild des Boxplots:
...
.question[Wie hängt die Arbeitslosenquote in den einzelnen Landkreisen mit deren BIP-Wachstum zusammen?]
