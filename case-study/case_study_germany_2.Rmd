...
Hier sehen wir insbesondere für Bayern und Baden-Württemberg Arbeitslosenquoten durchschnittlich unter 4% und für Sachsen-Anhalt, Bremen und Mecklenburg-Vorpommern Arbeitslosenquoten von durchschnittlich über 8%. Es fällt weiterhin auf, dass die ehemaligen ostdeutschen Bundesländer alle sehr hohe Arbeitslosenquoten aufweisen. Weiterhin ist die Standardabweichung, d.h. die Streuung um den Mittelwert, insgesamt für die einzelnen Bundesländer vergleichbar. Mit rund 1-2 Prozentpunkten ist die Standardabweichung auch recht hoch und deutet auf eine nicht unerhebliche Streuung unter den verschiedenen Landkreisen hin. Der Median für die Arbeitslosenquote liegt recht nahe am Mittelwert, was darauf hindeutet das es in den einzelnen Bundesländern nicht viele Landkreise gibt die von sehr hoher Arbeitslosigkeit geprägt sind.
...
Für alle Bundesländer in denen wir niedrige Arbeitslosenquoten sehen gibt es auch ein hohes BIP pro Kopf. D.h. speziell in unserem Datensatz von 2017 ist in den Bundesländern mit einer hohen Wirtschaftsleistung die Arbeitslosenquote gering. Insbesondere bei den ostdeutschen Bundesländern hat keines ein BIP pro Kopf über 30 000€, bei den westdeutschen Bundesländern liegt keines **unter** 30 000€. Bei den Schulden ist dieser Sachverhalt weniger deutlich ausgeprägt. Es gibt westdeutsche Bundesländer, wie z.B. Bayern und Baden-Württemberg mit sehr geringen Schulden pro Kopf, doch dies trifft auch für Sachsen oder Thüringen zu. Insbesondere das Saarland hat eine sehr hohe Verschuldung pro Kopf von durchschnittlich fast 6 000€.
...
Eine weitere Möglichkeit die Wachstumspfade zu visualisieren wäre mit sogenannten "small multiples". Hierdurch können wir die Aufmerksamkeit auf den Wachstumspfad richten und ignorieren dabei die Höhe des Startwertes: 
```{r}
...
Auch in diesem Beispiel sehen wir, dass alle Bundesländer über die Jahre hinweg ihr BIP pro Kopf steigern konnten und das der Effekt der Finanzkrise tendenziell in den westdeutschend Bundesländern ausgeprägter war als in den ostdeutschen Bundesländern.
Noch besser können wir den Effekt der Finanzkrise an der folgenden Grafik sehen, bei welcher die Veränderung des BIP in jedem Landkreis zum Vorjahr abgetragen wird:
...
Durch die drei vorherigen Schaubilder sehen wir, dass die Unterschiede zwischen den ost- und westdeutschen Landkreisen schon lange bestehen (mind. seit 2000) und das es keine große Annäherung der ostdeutschen Landkreise an das westdeutsche BIP pro Kopf gibt. Die Wachstumspfade sind hierfür zu ähnlich. In der Übungsaufgabe beschäftigen Sie sich mit dem Einkommen auf Landkreisebene und können dort nachvollziehen, ob der Trend bei den Einkommen der Gleiche ist wie bei dem BIP pro Kopf!

Wenn wir uns nur auf das Jahr 2017 beschränken, so sehen wir auch hier im BIP pro Kopf deutliche Unterschiede. Im folgenden Schaubild ist die Verteilung des BIP pro Kopf über alle Landkreise für Ost- und Westdeutschland im Jahr 2017 abgetragen bei denen das BIP pro Kopf unter 100 000€ lag. Zusätzlich sind die jeweiligen Mittelwerte des BIP pro Kopf für Ost- und Westdeutschland in blau eingezeichnet. Zum Einen können wir uns die Verteilung des BIP pro Kopf gut durch ein Histogramm anzeigen lassen, wobei die "binwidth", d.h. die Dicke eines Balkens, auf 1000€ beschränkt ist. Dies bedeutet das alle Beobachtungen innerhalb von 1000€ zusammengefasst in einem Balken dargestellt werden. Z.B. gibt es 10 Landkreise in Westdeutschland, die ein BIP pro Kopf zwischen 26000€ - 27000€ ausweisen (siehe Schaubild).
...
Was wir in diesem Fall tun können um die Vergleichbarkeit zu erhöhen ist die Verteilungsdichte zu zeigen. Im Grunde können Sie sich die Dichte als geglättetes Histogramm vorstellen, welche auf der y-Achse nicht mehr die Anzahl an Beobachtungen, sondern die Dichte abträgt.
...
Wenn wir uns die Korrelation zwischen den zwei Variablen ausgeben lassen, so erhalten wir die erwartete (schwach) negative Korrelation:
...
korrelationen <- gesamtdaten %>%
  ...
  mutate(rowname = c("1. BIP Wachstum", "2. Anteil Schulden", "3. Arbeitslosenquote")) %>% # Umbenennen der Variablennamen
